...
(function () {
  ...
  NS.createReader = function createReader(host, options) {
    ...
    function render() {
      ...
      if (span > 0) {
        var before = tokens.slice(Math.max(0, index - span), index);
        var after = tokens.slice(index + 1, index + 1 + span);

        var left = Rsvp.joinTokens(before);
        var previous = tokens[index - 1];
        if (previous && previous.wordIndex === token.wordIndex) {
          // The previous token is an earlier chunk of the word we're on, so its
          // trailing hyphen is noise — the rest of the word is right there.
          // The hyphen stays on the focused word itself, where it still means
        ...
        var right = Rsvp.joinTokens(after);
        if (right && !/-$/.test(text)) right = ' ' + right;

        ctxLeft.textContent = left;
        ctxRight.textContent = right;
      } else {
        ctxLeft.textContent = '';
        ctxRight.textContent = '';
      }
